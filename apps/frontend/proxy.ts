...
export async function proxy(request: NextRequest) {
   ...
   if (
      !role &&
      (pathname.startsWith("/screenshots") || pathname.startsWith("/dashboard"))
   ) {
...
function handleTokenBasedRouting(
   token: string,
   role: string,
   pathname: string,
   request: NextRequest
   ...
   if (
      !token &&
      (pathname.startsWith("/dashboard") || pathname.startsWith("/screenshots"))
   ) {
      return NextResponse.redirect(new URL("/login", request.url))
   }
